...
const ARTIC_ORGANIZATION_SCHEME string = "artic"
const ARTIC_OBJECT_TEMPLATE = "https://www.artic.edu/artworks/{objectid}/"

type ArtICSource struct {
	Source
...
func init() {
	...
	RegisterSource(ctx, ARTIC_ORGANIZATION_SCHEME, NewArtICSource)
}

func NewArtICSource(ctx context.Context, uri string) (Source, error) {

	t, err := uritemplates.Parse(ARTIC_OBJECT_TEMPLATE)
...
func (s *ArtICSource) Import(ctx context.Context, db database.AccessionNumberDatabase, uris ...string) error {
...
func (s *ArtICSource) ObjectURI(ctx context.Context, acc *database.AccessionNumber) (string, error) {
...
func (s *ArtICSource) importURI(ctx context.Context, db database.AccessionNumberDatabase, u string) error {
...
func (s *ArtICSource) importReader(ctx context.Context, db database.AccessionNumberDatabase, r io.Reader) error {
	...
	str_id := strconv.Itoa(int(object_id.(float64)))

	accession_number, ok := row["main_reference_number"]

	if !ok {
		return fmt.Errorf("Row is missing main_reference_number column")
	}

	org_uri := fmt.Sprintf("%s://", ARTIC_ORGANIZATION_SCHEME)
